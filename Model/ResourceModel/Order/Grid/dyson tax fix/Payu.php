...
class Payu extends AbstractHelper
{
    ...
    public function setDiscount($order, $customDiscount, $discountDescription)
    {
        ...
        foreach ($order->getAllItems() as $item) {
             if($item->getProductType() !="bundle" && $item->getProductType() !="soft") {
                     ...
                     { 
                        ...
                        $discountTaxPart= ($itemDiscountAmount * $item->getTaxPercent()) / (100 + $item->getTaxPercent());
                        $totalDiscountTaxPart += $discountTaxPart;
                        $item->setTaxAmount($taxAmount - $discountTaxPart);
                        $item->setBaseTaxAmount($taxAmount - $discountTaxPart);
                        $item->setDiscountTaxCompensationAmount(
                            $item->getDiscountTaxCompensationAmount() + $discountTaxPart
                        );
                        $item->setBaseDiscountTaxCompensationAmount(
                            $item->getBaseDiscountTaxCompensationAmount() + $discountTaxPart
                        );
                        $item->save();
                     }
             }
        ...
        $order->setTaxAmount($order->getTaxAmount() - $totalDiscountTaxPart);
        $order->setBaseTaxAmount($order->getBaseTaxAmount() - $totalDiscountTaxPart);
        $order->setDiscountTaxCompensationAmount(
            $order->getDiscountTaxCompensationAmount() + $totalDiscountTaxPart
        );
        $order->setBaseDiscountTaxCompensationAmount(
            $order->getBaseDiscountTaxCompensationAmount() + $totalDiscountTaxPart
        );
        $order->save();
    }
